...
app.http("httpTrigger1", {
  ...
  handler: async (request, context) => {
    context.log(
      `[fc_gp_cloudInn_update] Processando requisição ${request.method} para "${request.url}"`,
    );
    ...
    try {
      ...
      const idParam =
        ...
        query.get("roomId") ||
        body.id;
      const statusParam = query.get("status") || body.status;
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          message:
            error.message ||
            "Erro inesperado ao atualizar registro no MongoDB.",
        }),
      };
    }
  },
