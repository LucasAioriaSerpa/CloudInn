...
async function handler(request, context, options = {}) {
  ...
  try {
    ...
    if (!mongoUri) {
      ...
      log(
        `[fc_gp_cloudInn_update] AVISO: URI do MongoDB não encontrada ou inválida. Variáveis com nomes relacionados no Azure: ${checkedKeys.length > 0 ? checkedKeys.join(", ") : "nenhuma"}`,
      );
      return {
        status: 500,
        headers: corsHeaders,
    ...
    log(`[fc_gp_cloudInn_update] Conectando ao MongoDB: ${maskMongoUri(mongoUri)}`);
