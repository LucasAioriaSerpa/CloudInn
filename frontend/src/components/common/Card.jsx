...
export function CardHeader({ title, subtitle, action, className = "" }) {
  return (
    <div
      className={`p-5 pb-3 flex items-start justify-between border-b border-[#F9F5FF] ${className}`}
    >
      <div>
        <h3 className="text-base font-bold text-[#28262C] font-heading">
          {title}
        </h3>
        {subtitle && (
          <p className="text-xs text-[#28262C]/65 mt-0.5">{subtitle}</p>
        )}
      </div>
